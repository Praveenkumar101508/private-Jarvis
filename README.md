...
IRA runs **native on Windows** (Shadow PC) — Ollama + the Cortex gateway, no Docker.
...
# 3 — Start the full native stack (Postgres, Memurai, Ollama, Cortex gateway,
...
- **Engine** — voice uses Ollama `qwen3:8b` (fast) by default, or Cortex when
  `IRA_USE_CORTEX=true`. Both work; direct-Ollama has the lowest latency.

**One command to start everything** (native Windows, no Docker):

```powershell
pwsh -File .\start-ira.ps1                     # Postgres → Redis → Ollama → Cortex → Supertonic → API → frontend
