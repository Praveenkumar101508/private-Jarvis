...
- **Brain ↔ Memory** — reasons through the local **Hermes** agent gateway (Ollama
  models), and *remembers*: each conversation carries a stable thread id
  (`X-Hermes-Session-Id`) and a per-owner long-term memory scope
  (`X-Hermes-Session-Key`). An **owner profile** (name / goals / projects /
...
Every model and backend is **local / self-hosted**: Ollama for inference, the Hermes
...
Hermes URL or a research backend is ever pointed off-box, and the public-only guard
...
copy .env.example .env          # fill in the secrets + IRA_HERMES_KEY
...
Key env (see `.env.example`): `LLM_BACKEND=ollama`, `IRA_USE_HERMES=true`,
`IRA_HERMES_URL`/`IRA_HERMES_KEY`, `OLLAMA_*`, `SEARXNG_URL`/`CRAWL4AI_URL`,
