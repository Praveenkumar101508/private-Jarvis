...
belongs to Hermes. Schema: postgres/008_business_data.sql (tenants, investors, client_agents).
...
TENANT ISOLATION (no cross-tenant reads):
  ...
  - Hermes memory is isolated per tenant via the bridge's X-Hermes-Session-Key header —
...
def session_key_for_tenant(tenant_id: str) -> str:
    """Per-tenant Hermes memory scope (value for the X-Hermes-Session-Key header)."""
