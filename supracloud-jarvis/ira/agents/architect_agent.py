...
Compares IRA vs Grok/the assistant/Gemini/ChatGPT/DeepSeek continuously.
...
IRA_CAPABILITY_MAP = {
    "implemented": [
        ...
        "Video generation + video understanding (Replicate)",
        "Design tools — UI mockups, SVG, HTML prototypes",
        "Computer use / desktop agent (Playwright)",
        "Native document creation (PDF, PPTX, Excel, Word)",
        "Music and audio generation (MusicGen, Bark, SFX via Replicate)",
        "Deep research + long-form article generation",
        "Multi-modal analysis (vision + document + audio)",
    ],
    "missing": [
        "Persistent file storage — user-uploaded files saved between sessions",
        "Calendar write-back — IRA creates/edits events in Google Calendar / Cal.com",
        "Multi-user support — separate profiles, memories, and permissions per user",
        "Proactive email drafting — IRA monitors inbox and drafts replies for review",
        "Playwright auto-apply — automated job application submission pipeline",
        "Mobile push notifications — native iOS/Android alerts beyond Telegram",
        "Long-term goal tracking — weekly progress reports toward user-defined goals",
    ],
...
2. For each: what does Grok/the assistant/Gemini/ChatGPT do, and what gap does IRA have?
...
3. **🔍 Feature Details** — full description, how it compares to Grok/the assistant/Gemini version
    ...
    try:
        ...
        async for event in stream_architect_proposal(
            query=(
                ...
                "the assistant, Gemini, and ChatGPT released in the last cycle."
            ),
