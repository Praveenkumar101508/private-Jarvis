...
def test_public_url_allowed():
    assert is_public_url("https://example.com/page", resolve_fn=lambda h: ["93.184.216.34"])[0]
...
def test_private_and_internal_urls_blocked():
    for u in (
        ...
        ok, _reason = is_public_url(u, resolve_fn=lambda h: ["93.184.216.34"])
