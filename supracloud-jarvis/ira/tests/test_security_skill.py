"""Security skill reasoning through the Cortex bridge (Option A).

Skips unless the gateway is reachable AND IRA_CORTEX_KEY is set, so CI stays green
...
def test_security_reasoning_via_bridge():
    ...
    # NOTE: the Cortex gateway is agentic and (esp. on the security persona) will sometimes
