"""Phase 7.2 — the IRA_USE_HERMES feature flag routes requests between the legacy
LangGraph engine (run_graph) and the Hermes bridge (router gate -> skills via run_skill).
...
same _hermes_route, so it is exercised too.
"""
import os
...
def test_flag_off_routes_to_legacy_run_graph(monkeypatch):
    ...
    run_skill = MagicMock(return_value="hermes answer")
    ...
    assert not run_skill.called, "flag OFF must NOT invoke the Hermes bridge skill"
...
def test_flag_on_routes_to_bridge_skill(monkeypatch):
    ...
    assert run_skill.called, "flag ON must invoke the Hermes bridge skill"
    assert not run_graph.called, "flag ON must NOT invoke the legacy run_graph"
    assert resp.response == "hermes answer"
...
def test_flag_on_streaming_routes_to_bridge(monkeypatch):
    ...
    _hermes_route runs eagerly before the SSE response is returned, so the skill call is
