...
def _patch_probes(monkeypatch, *, pg, rd, ollama, cortex):
    ...
    monkeypatch.setattr(hmod, "_check_cortex", AsyncMock(return_value=ServiceStatus(status=cortex, latency_ms=1)))
...
def test_detail_reports_all_pillars_with_one_subsystem_down(monkeypatch):
    _patch_probes(monkeypatch, pg="down", rd="ok", ollama="ok", cortex="degraded")
...
def test_detail_all_down_is_down_and_still_returns_every_pillar(monkeypatch):
    _patch_probes(monkeypatch, pg="down", rd="down", ollama="down", cortex="down")
    out = asyncio.run(health_detail())
...
def test_detail_healthy_core_is_ok(monkeypatch):
    _patch_probes(monkeypatch, pg="ok", rd="ok", ollama="ok", cortex="ok")
    out = asyncio.run(health_detail())
