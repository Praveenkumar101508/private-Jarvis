"""Phase 4 — both engines (Ollama-direct and Hermes) return non-empty output, and
the voice path pins the fast (qwen3:8b) tier.

No real Ollama or Hermes is needed: the Ollama OpenAI client and the `hermes -z`
subprocess are mocked. This asserts the two bridges IRA toggles between with
IRA_USE_HERMES both produce a usable reply, and that fast/deep tiers resolve to the
...
# ── Ollama-direct engine (IRA_USE_HERMES=false) ───────────────────────────────
...
# ── Hermes engine (IRA_USE_HERMES=true) ───────────────────────────────────────

def test_hermes_engine_returns_nonempty(monkeypatch):
    """`hermes -z` (mocked subprocess) yields a non-empty final reply."""
    run = MagicMock(
    ...
    out = HermesBridge(HermesConfig()).ask("what time is it", system="Reply in one sentence.")
    assert out.strip(), "Hermes engine returned empty output"
