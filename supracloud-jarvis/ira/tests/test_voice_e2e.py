...
engine stubbed (no faster-whisper / supertonic / Hermes / Ollama / DB):
  1. POST /voice/transcribe  -> {text, is_owner}   (DEV_MODE owner)
  2. the transcript through the chat brain (Hermes path, skill stubbed) -> reply
...
def test_voice_round_trip_transcribe_chat_say(monkeypatch):
    ...
    # ── 2. Chat: feed the transcript through the brain (Hermes path, skill stubbed) ─
    ...
    reply = resp.response
    assert reply.strip() and resp.model_used == "hermes"
