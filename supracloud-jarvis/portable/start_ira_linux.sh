#!/usr/bin/env bash
# IRA Portable — one-command start (Linux).
...
command -v docker  >/dev/null 2>&1 || fail "docker is required (and the daemon must be running)."
...
if python3 "$HERE/health_check.py" --base-url "$API_BASE" --timeout "${IRA_READY_TIMEOUT:-120}"; then
    ...
    xdg-open "$FRONTEND_URL" >/dev/null 2>&1 || info "Open $FRONTEND_URL in your browser."
    info "IRA is up."
else
