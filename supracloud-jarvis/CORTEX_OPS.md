# CORTEX_OPS.md — Cortex operational runbook & Phase-6 hardening

Cortex runs **out-of-process** (native Windows install under `%LOCALAPPDATA%\cortex`),
behind an OpenAI-compatible gateway on `127.0.0.1:8642`. IRA reaches it ONLY through
`ira/cortex_bridge.py`. This file is the bank-build hardening + upgrade runbook.

## Security posture (verified 2026-06-03)
- **Gateway: `127.0.0.1:8642`, key-gated** (`API_SERVER_ENABLED=true`, `API_SERVER_HOST=127.0.0.1`,
  `API_SERVER_KEY` in `~/.cortex/.env`, never in the repo). ✓ Localhost-only entry point.
...
- **Red-team/jailbreak skills stripped** for the bank build — `scripts/strip-bank-skills.ps1`
  ...
  `~/.cortex/skills_disabled/`. Re-run after any `cortex` install/update.
- **✓ RESOLVED (Phase 7.1) — reasoning-only gateway profile.** The `api_server` agent now has its
  **entire toolset DISABLED** (zero tools) at the config level (`cortex tools disable --platform
  ...
  bled into reasoning** (architect agents referenced a Cortex task list / SKILL.md instead of answering),
  ...
  directive stays as defense-in-depth. Reproduce: `scripts/harden-gateway.ps1`; re-run after any `cortex`
  update (a fresh install resets tool config).

## Vendor freeze (disaster recovery / certified copy)
- `cortex-vendor/CHECKSUMS.txt` pins `hermes-agent==0.15.2` + the wheel sha256.
- `scripts/freeze-cortex-vendor.ps1` (re)downloads + verifies the wheel; the wheel is gitignored
  (kept on-machine, not in git history).

## Upgrade procedure (the "rehearsal")
Because Cortex is **out-of-process and isolated behind the HTTP/OpenAI bridge, a Cortex
...
2. Update `cortex-vendor/CHECKSUMS.txt` + `scripts/freeze-cortex-vendor.ps1` (`$pin` + `$expected`).
3. Re-run `scripts/strip-bank-skills.ps1` (a fresh install re-syncs bundled skills).
4. Restart the gateway (`cortex gateway` / the Startup launcher) and run the overlay tests.

`ira/requirements.txt` does NOT contain `hermes-agent`, so the CI (`ci.yml`, which installs
`ira/requirements.txt` + runs the overlay tests) is **unaffected by Cortex version bumps** — the
