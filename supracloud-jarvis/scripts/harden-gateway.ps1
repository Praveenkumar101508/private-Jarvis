...
#
# Two fixes that MUST be in place before IRA_USE_CORTEX is ever flipped ON:
...
#   2. Make the Cortex API-server agent REASONING-ONLY — remove file/shell/web/exec/
...
#
# Idempotent. Re-run after any `cortex` install/update (a fresh install resets tool config).
$ErrorActionPreference = "Stop"

$cortex = Join-Path $env:LOCALAPPDATA "hermes\hermes-agent\venv\Scripts\hermes.exe"
if (-not (Test-Path $cortex)) { throw "hermes.exe not found at $cortex" }
...
#        BLED into reasoning (7.3 live verification: architect agents referenced a Cortex "task
...
& $cortex tools disable --platform api_server @toolsets
Write-Host "OK: api_server agent is reasoning-only (ZERO toolsets enabled)."
Write-Host "Verify: cortex tools list --platform api_server   (expect no 'enabled' lines)"
Write-Host "Undo:   cortex tools enable --platform api_server <toolset> ..."
